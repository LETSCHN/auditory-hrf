...
%
% Input: smoothed_data_cutoff1500_ERA_SD_CORR85.mat, containing
% all_reps_eras_norm from the preprocessing scripts.
% Output: training_s0X_sessZ_trialeffect.mat files, containing fitted
% model parameters, best-model labels, adjusted R-squared values, and train.
% Used afterwards by plot_trial_effect_trajectories.m.
%
% Single-session version. The selected session is controlled by the sess
% variable below; it is currently set to session 2.
...
outfile = '_trialeffect.mat'; % training_s0X_sessZ_outfile

subjects = 1:5;

nRunsPerSession = 6;
nSessions = 1; % total sessions
sess = 2; % only this session

xx = 0:18; % time axes of data & fits

% ================== data combinations, check carefully, add more combinations as needed

% combinations in selected session
...
for s = subjects
    ...
    all_data = all_reps_eras_norm{s,sess};

    rsq_overall = []; bm_overall = [];
...
%                 subplot(4,2,8); plot(f6{itr},xx,data); title(['m6: ' num2str(rsq(6,itr,i))]); grid on; hold on; legend('off')
    ...
    save([inpath '/training_s0' num2str(s) '_sess' num2str(sess) outfile],'m1','m2','m3','m4','m5','m6',...
