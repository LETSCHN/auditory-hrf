...
for s = subject
    for roi = 1:nROI
        ...
        if nvox > vox_thresh - 1
            ...
            if ~checkatBoundM3_8(f6_allsess,upper_model6,lower_model6), rsq_allsess(6) = gof6_allsess.adjrsquare; end
...
writematrix(concatenated_data_peak, [outpath '/s0' num2str(subject) '_' hemi{1} '_peak_Marty_ROIs_bm.txt'], 'Delimiter', 'tab');
writematrix(concatenated_data_fwhm, [outpath '/s0' num2str(subject) '_' hemi{1} '_fwhm_Marty_ROIs_bm.txt'], 'Delimiter', 'tab');
writematrix(concatenated_data_amp,  [outpath '/s0' num2str(subject) '_' hemi{1} '_amp_Marty_ROIs_bm.txt'], 'Delimiter', 'tab');
