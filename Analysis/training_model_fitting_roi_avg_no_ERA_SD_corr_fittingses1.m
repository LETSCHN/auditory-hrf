...
outpath = inpath;
hemi = {'lh'};% {'rh'}
outfile = 'fittedAmpPeakFWHM_ROIs_cutoff1500_sess1.mat';

roidir = ['/Users/letitia/Dropbox/HiHi/ROIs/ROIs'];
roi_names = { 'CL', 'CM', 'CP','C_A4','C_A5', 'M_A4', 'M_A5', 'MPr', 'MPc', 'AL', 'ML', 'MM', 'A1', 'R_A4', 'R_A5', 'RT', 'RTL', 'R', 'RM','RP', 'TA2', 'TA3'};
...
data_sess1_allROIs = cell(nROI, 1);
...
%                 disp(['Session: ' num2str(sess)]);
            ...
            % === ERA for session 1 ===
            if ~isempty(all_reps_eras_norm{roi, 1})
                temp = squeeze(mean(all_reps_eras_norm{roi, 1}, 2));
                data_sess1 = mean(temp, 1);
            else
                data_sess1 = nan(1, length(xx));
            end

            % Store both versions
            data_sess1_allROIs{roi} = data_sess1;

            % === Fit models to session 1 ===
            [f1_sess1, gof1_sess1] = fit(xx', data_sess1', model1, 'StartPoint', start_model1, 'Lower', lower_model1, 'Upper', upper_model1);
            [f2_sess1, gof2_sess1] = fit(xx', data_sess1', model2, 'StartPoint', start_model2, 'Lower', lower_model2, 'Upper', upper_model2);
            [f3_sess1, gof3_sess1] = fit(xx', data_sess1', model3, 'StartPoint', start_model3, 'Lower', lower_model3, 'Upper', upper_model3);
            [f4_sess1, gof4_sess1] = fit(xx', data_sess1', model3, 'StartPoint', start_model4, 'Lower', lower_model4, 'Upper', upper_model4);
            [f5_sess1, gof5_sess1] = fit(xx', data_sess1', model3, 'StartPoint', start_model5, 'Lower', lower_model5, 'Upper', upper_model5);
            [f6_sess1, gof6_sess1] = fit(xx', data_sess1', model3, 'StartPoint', start_model6, 'Lower', lower_model6, 'Upper', upper_model6);

            % === R² values for session 1 ===
            rsq_sess1 = nan(1,6);
            if ~checkatBoundM1(f1_sess1,upper_model1,lower_model1), rsq_sess1(1) = gof1_sess1.adjrsquare; end
            if ~checkatBoundM2(f2_sess1,upper_model2,lower_model2), rsq_sess1(2) = gof2_sess1.adjrsquare; end
            if ~checkatBoundM3_8(f3_sess1,upper_model3,lower_model3), rsq_sess1(3) = gof3_sess1.adjrsquare; end
            if ~checkatBoundM3_8(f4_sess1,upper_model4,lower_model4), rsq_sess1(4) = gof4_sess1.adjrsquare; end
            if ~checkatBoundM3_8(f5_sess1,upper_model5,lower_model5), rsq_sess1(5) = gof5_sess1.adjrsquare; end
            if ~checkatBoundM3_8(f6_sess1,upper_model6,lower_model6), rsq_sess1(6) = gof6_sess1.adjrsquare; end

            [rsq_overall,bm_overall] = max(rsq_sess1(:));
            disp(['roi: ' roi_names{roi} ', bm: ' num2str(bm_overall) ', rsq-adj: ' num2str(rsq_overall)]);

            subplot(4,2,1);
            plot(xx, data_sess1, 'ko:');
            title('Session 1 ERA'); grid on; 

            for m = 1:6
                subplot(4,2,m+2);
                plot(eval(['f' num2str(m) '_sess1']), xx, data_sess1);
                title(['m' num2str(m) ': ' num2str(rsq_sess1(m))]); grid on; legend('off');
            end
            %pause
            ...
            if rsq_overall > rsq_thresh
                if bm_overall < 5
                    temp_peak = eval(['f' num2str(bm_overall) '_sess1.peak1']);
                    temp_fwhm = eval(['f' num2str(bm_overall) '_sess1.fwhm1']);
                    temp_amp = eval(['f' num2str(bm_overall) '_sess1.A']);
                else
                    temp_peak = eval(['f' num2str(bm_overall) '_sess1.peak2']);
                    temp_fwhm = eval(['f' num2str(bm_overall) '_sess1.fwhm2']);
                    temp_amp = eval(['f' num2str(bm_overall) '_sess1.A']) * eval(['f' num2str(bm_overall) '_sess1.ratio1']);
                end
            else
                temp_peak = 0; 
                temp_fwhm = 0;
