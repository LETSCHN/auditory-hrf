% Fit HRF models to session 2 cluster-mean ERA time courses.
%
% The cluster means are fitted with:
% 1. canonical two-gamma HRF,
% 2. canonical HRF plus temporal and dispersion derivatives,
% 3. GLMsingle HRF library curves,
% 4. six custom gamma-model families.
%
% This script is used to inspect all cluster fits and to generate the
% adjusted R-squared values summarised in Cluster_means_fits_Rsq_ses2_March2026.xlsx.
% The selected examples are plotted for the figure by
% plot_cluster_mean_hrf_model_fits.m.
%
...
for i = 1:size(clusters,1)
    ...
    disp(['Best GLM single basis: ' num2str(best_basis) ', R-sq (adj): ' num2str(max_rsq)]);
        plot(glm_single(best_basis,:)); %if you want to see how it looks
    ...
    custom_model_bold = custom_model_bold(xx+1);%rescale(custom_model_bold(xx+1),min(custom_model_bold),max(data)); Rescale not needed as amp already fitted in model
