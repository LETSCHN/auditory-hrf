...
% subjects in JMP. The means are fitted to (a) Canonical 2 gamma (default), (b) Canonincal 2
...
% IZ (2025)
% Updated in Sep 2025 by LS to create a subplot on selected clusters instead of
% plotting all individually
% Updated in March 2026 to estimate amplitudes using beta estimation instead of
% rescaling
...
selected_clusters = [1, 16, 21, 28, 35, 39];%without glmsingle[3, 17, 21, 28, 29, 35]; %[5, 8, 19, 31, 33, 40] orig (ses1)
...
for idx = 1:length(selected_clusters)
    ...
    pastel_black = [0.3 0.3 0.3];       % softer black
    pastel_blue  = [0.4 0.6 0.8];       % pastel blue
    pastel_green = [0.5 0.8 0.5];       % pastel green
    pastel_magenta = [0.8 0.6 0.8];     % pastel magenta
    pastel_red   = [0.9 0.5 0.5];       % pastel red
    pastel_white = [1 1 1];             % white remains unchanged

    % Plotting   
    p1 = plot(xx, data, 'o'); box off;
    set(p1, {'Color', 'LineWidth'}, {pastel_black, 5});
    hold on;

    p2 = plot(xx, canonical_bold); box off;
    set(p2, {'Color', 'Linestyle', 'LineWidth'}, {pastel_blue, '--', 5});

    p3 = plot(xx, bfs_bold); box off;
    set(p3, {'Color', 'Linestyle', 'LineWidth'}, {pastel_green, '--', 5});

    p4 = plot(xx,glm_single(best_basis,:));
    set(p4,{'Color','Linestyle','LineWidth'},{pastel_magenta,'--',5});

    p5 = plot(xx,custom_model_bold);
    set(p5,{'Color','Linestyle','LineWidth'},{pastel_red,'--',5});
    ...
    ylabel('Signal Change (\%)', 'FontSize', 14, 'FontWeight', 'bold')

    % Axis formatting logic — must come AFTER plotting
    row = ceil(idx / 3);           % 3 columns per row
    col = mod(idx-1, 3) + 1;       % column index (1 to 3)

    % Remove x-axis from row 1
    if row == 1
        set(gca, 'XTickLabel', []);
        xlabel('');
    end
    ...
    % Y-axis label logic: show on both left column plots
    if col == 1
        ylabel('Signal Change (%)', 'FontSize', 16, 'FontWeight', 'bold');
    else
        set(gca, 'YTickLabel', []);
        ylabel('');
    end

    % X-axis label logic: show on all bottom row plots
    if row == 2
    ...
    % Tick font size: increase where ticks are shown
    if col == 1 || row == 2
        set(gca, 'FontSize', 18);
    else
        set(gca, 'FontSize', 1);  % visually hide ticks
    end
    ...
    if idx == length(selected_clusters)
        legend({'Data',...
            ['Canonical HRF'],...
            ['Informed basis set'],...
            ['GLM single'],...
    ...
    % Store results using idx
    result_models(idx,1) = bm(i);
    result_models(idx,2) = rsq_custom(i);

    disp(['Cluster ' num2str(i) ': Best model = ' num2str(result_models(idx,1)) ...
        ', R-sq (adj) = ' num2str(result_models(idx,2))]);
